defmodule Phoenix.Channel.Client.Socket do
    ...
    quote unquote: false do
      ...
      def start_link() do
        GenServer.start_link(
          Phoenix.Channel.Client.Socket,
          {unquote(__MODULE__), unquote(var!(config))},
          name: __MODULE__
        )
      end
  ...
  def handle_info(:heartbeat, %{state: connected} = state) do
    ...
    send(
      state.socket,
      {:send, %{topic: "phoenix", event: "heartbeat", payload: %{}, ref: to_string(ref)}}
    )
  ...
  def handle_info({:closed, reason}, state) do
    ...
    Enum.each(state.channels, fn {pid, _} ->
      send(pid, {:trigger, :phx_error, reason, state.ref})
    end)
